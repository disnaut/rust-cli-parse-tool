...
// I wonder if this is doing iteration 1 but with an extra step with the buffer.
fn iteration_three() {
    let mut rng = rand::rng();
...
// Took 1 min 7 seconds.
// flushing after each write. Not sure what's going on here memory-wise.
fn iteration_four() {
    let mut rng = rand::rng();
...
// Flushes buffer every 100 lines.
// 30.0 seconds to create 1gb file
fn iteration_five() {
    let mut rng = rand::rng();
